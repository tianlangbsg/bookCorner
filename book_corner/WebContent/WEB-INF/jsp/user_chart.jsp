...
$(document).ready(function() {
	...
	function init() {
		var chart_type =<%=request.getAttribute("user_chart_type")%>;
		if (chart_type != "null") {
			if (chart_type == 1) {
...
<body onload="init()">
	...
	<h1 align="center">用户排行</h1>
	<div align="center">
		<table style="width: 600px">
			<tr>
				<td><form id="form1" method="post"
						action="/book_corner/userChartBorrow">
						<div align="center">
							<input type="submit" name="button1" id="button1"
								class="btn btn-lg btn-primary btn-block" value="阅读数排行(降序)" />
						</div>
					</form></td>
				<td><form id="form1" method="post"
						action="/book_corner/userChartJoinAtAsc">
						<div align="center">
							<input type="submit" name="button2" id="button2"
								class="btn btn-lg btn-primary btn-block" value="加入时间（升序）" />
						</div>
					</form></td>
				<td><form id="form1" method="post"
						action="/book_corner/userChartJoinAtDesc">
						<div align="center">
							<input type="submit" name="button3" id="button3"
								class="btn btn-lg btn-primary btn-block" value="加入时间（降序）" />
						</div>
					</form></td>
			</tr>
		</table>
				...
				<tbody>
					<c:forEach items="${users}" var="user">
						<tr>
							...
							<td width="20%"><a href="/book_corner/userBookList?userId=${user.userId}">${user.userFullname}</a></td>
							<td width="20%"><div align="center">${user.joinAt}</div></td>
							<td width="20%"><div align="center">${user.borrowCount}</div></td>
							<td width="20%"><div align="center">${user.lastBorrowDate}</div></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
